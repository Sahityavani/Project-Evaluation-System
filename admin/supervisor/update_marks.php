...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $evaluation_type = isset($_POST['evaluation_type']) ? (int)$_POST['evaluation_type'] : 0;
     $supervisor_id = isset($_POST['evaluator_id']) ? (int)$_POST['evaluator_id'] : 0;
    $sub_parts_marks = isset($_POST['marks']) ? $_POST['marks'] : [];
    $remarks = isset($_POST['remarks']) ? $_POST['remarks'] : [];

    if (!$evaluation_type || empty($sub_parts_marks)) {
    ...
    foreach ($sub_parts_marks as $student_id => $student_marks) {
        ...
        $sql = "
            ...
            WHERE evaluation_type = ? AND student_id = ? AND supervisor_id = ?
        ";
        $stmt = $conn->prepare($sql);

        // Bind parameters: s (string) for sub_parts_json, i (integer) for totalMarks, s (string) for remarks, s (string) for evaluation_date, i (integer) for evaluation_type, i (integer) for student_id
        $stmt->bind_param('sissiis', $sub_parts_json, $totalMarks, $remark, $evaluation_date, $evaluation_type, $student_id, $supervisor_id);
