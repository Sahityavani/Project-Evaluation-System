...
// Get evaluation type from POST data
...
if ($result->num_rows > 0) {
    ...
    $supervisor_id = $row_evaluator['id'];
} else {
...
if ($result_max->num_rows > 0) {
    ...
    $maxSubParts = json_decode($row_max['sub_parts_supervisor'], true);
} else {
    echo 'No sub-parts configuration available for this evaluation type.';
    exit();
}

// Fetch current marks and sub_parts details from supervisor_evaluations
$sql_marks = "
    SELECT e.student_id, e.marks, e.remarks, s.rollno, s.name, e.sub_parts
    FROM supervisor_evaluations e
    JOIN students s ON e.student_id = s.id
    WHERE e.evaluation_type = ? AND e.supervisor_id = ?
";
$stmt_marks = $conn->prepare($sql_marks);
$stmt_marks->bind_param('is', $evaluation_type, $supervisor_id);
$stmt_marks->execute();
...
if ($result_marks->num_rows > 0) {
    ...
    echo '<input type="hidden" name="evaluator_id" value="' . htmlspecialchars($supervisor_id) . '">';
