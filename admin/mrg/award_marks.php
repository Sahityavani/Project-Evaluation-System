...
$sql = "SELECT student_id FROM mrg_evaluations 
...
$sql = "SELECT MAX(mrg_evaluation_id) as max_id FROM mrg_evaluations";
$result = $conn->query($sql);
$max_id_row = $result->fetch_assoc();
$mrg_evaluation_id = $max_id_row['max_id'] + 1;
...
foreach ($sub_parts as $student_id => $parts) {
    ...
    $insert_sql = "INSERT INTO mrg_evaluations 
    (mrg_evaluation_id, batch_id, evaluator_id, student_id, marks, remarks, evaluation_date, student_rollno, evaluation_type, sub_parts)
    VALUES 
    ($mrg_evaluation_id, $batch_id, $evaluator_id, $student_id, $total_marks, '$student_remarks', '$evaluation_date', '$student_rollno', $evaluation_type, '$sub_parts_json')";
