...
// Retrieve current evaluator's ID
...
if (!$evaluator) {
    $response['message'] = 'Evaluator not found for username: ' . htmlspecialchars($username);
    echo json_encode($response);
    exit;
}
...
// Prepare SQL query to fetch students and their previous marks from mrg_evaluations
$sql = "SELECT s.id, s.name, s.rollno, 
               IFNULL(m.sub_parts, '{}') AS prev_marks
        FROM students s
        LEFT JOIN mrg_evaluations m ON s.id = m.student_id 
...
$stmt->bind_param('iii', $previous_evaluation_type, $evaluator_id, $batch);
$stmt->execute();
$result = $stmt->get_result();
