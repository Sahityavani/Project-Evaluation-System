<?php
    include 'auth.php';
    $title = "Evaluator | Add/Edit Marks";
    include '../head.php';
    include '../db.php';
    ...
    $sql = "SELECT * FROM evaluators WHERE username = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        // 2. Get the MRG ID for the Evaluator
        $sql = "SELECT mrg_id FROM mrg_evaluators WHERE evaluator_id = ?";
        $stmt = $conn->prepare($sql);
        ...
        if ($mrgResult->num_rows > 0) {
            $row = $mrgResult->fetch_assoc();
            $mrgId = $row['mrg_id'];

            // 3. Get Batch Details directly from MRG ID
            $sql = "SELECT b.*, m.mrg_name, GROUP_CONCAT(DISTINCT e.name SEPARATOR ', ') AS evaluators
                    FROM batches b
                    LEFT JOIN mrg m ON b.mrg_id = m.mrg_id
                    LEFT JOIN mrg_evaluators me ON m.mrg_id = me.mrg_id
                    LEFT JOIN evaluators e ON me.evaluator_id = e.evaluator_id
                    WHERE b.mrg_id = ?
                    GROUP BY b.batch_id, m.mrg_name";
            ...
            echo 'No Batch Alloted For Evalaution.';
        }
    }

    // Fetch evaluation types
    $evaluation_types = [];
    $sql = "SELECT * FROM evaluation_type where type='MRG'";
