<?php
    ...
    if ($result->num_rows > 0) {
        ...
        // 2. Get the DPEG ID for the Evaluator
        $sql = "SELECT dpeg_id FROM dpeg_evaluators WHERE evaluator_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('i', $evaluatorId);
        $stmt->execute();
        $dpegResult = $stmt->get_result();

        if ($dpegResult->num_rows > 0) {
            $row = $dpegResult->fetch_assoc();
            $dpegId = $row['dpeg_id'];

            // 3. Get Batch Details directly from DPEG ID
            $sql = "SELECT b.*, m.dpeg_name, GROUP_CONCAT(DISTINCT e.name SEPARATOR ', ') AS evaluators
                    FROM batches b
    ...
    $evaluation_types = [];
    $sql = "SELECT * FROM evaluation_type where type='DPEG'";
