<?php
include('auth.php');
$title = "Admin | View MRG Evaluators";
include('../head.php');
include('../db.php');
...
$evaluatorsQuery = "
    SELECT e.evaluator_id, e.name, e.username, m.mrg_id
    FROM evaluators e
    LEFT JOIN mrg_evaluators me ON e.evaluator_id = me.evaluator_id
    LEFT JOIN mrg m ON me.mrg_id = m.mrg_id
";
$evaluatorsResult = $conn->query($evaluatorsQuery);

// Fetch MRG details for a specific MRG
$mrgDetails = [];
if (isset($_GET['mrg_id'])) {
    $mrgId = intval($_GET['mrg_id']);
    $mrgQuery = "
        SELECT m.mrg_name, m.mrg_title
        FROM mrg m
        WHERE m.mrg_id = $mrgId
    ";
    $mrgResult = $conn->query($mrgQuery);
    if ($mrgResult->num_rows > 0) {
        $mrgDetails = $mrgResult->fetch_assoc();
    }
}
?>
        ...
        <div class="main-content">
            <div class="page-content">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
                                <h4 class="mb-sm-0">MRG Evaluators</h4>
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        <li class="breadcrumb-item"><a href="index.php">Home</a></li>
                                        <li class="breadcrumb-item active">View MRG Evaluators</li>
                                    </ol>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- MRG Details (if applicable) -->
                    <?php if ($mrgDetails): ?>
                        <div class="card mt-4">
                            <div class="card-header">
                                <h4 class="card-title mb-0">MRG Details</h4>
                            </div>
                            <div class="card-body">
                                <p><strong>MRG Name:</strong> <?php echo htmlspecialchars($mrgDetails['mrg_name']); ?></p>
                                <p><strong>MRG Title:</strong> <?php echo htmlspecialchars($mrgDetails['mrg_title']); ?></p>
                            </div>
                        </div>
                    <?php endif; ?>
                                    ...
                                    <tbody>
                                        <?php while ($evaluator = $evaluatorsResult->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td><?php echo htmlspecialchars($evaluator['mrg_id'] ?: 'Not Assigned'); ?></td>
                                            </tr>
                                        <?php endwhile; ?>
                                    </tbody>
                                </table>
